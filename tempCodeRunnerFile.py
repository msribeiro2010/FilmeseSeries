        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            ...
            for movie in tqdm(movies, desc="Fetching movie details", total=len(movies)):
                ...
                if release_year >= start_year and release_year <= end_year:
                    ...
                    futures.append((numeric_movie_id, [title, release_year, rating]))
                
                if len(futures) >= num_movies:
                    break
        
        for movie_id, movie in futures:
...
def main():
    start_year = 2021
    end_year = 2023
    num_movies = 50  # Fetch top 1000 movies
    movie_data = get_top_imdb_movies(start_year, end_year, num_movies)
    movie_data.sort(key=lambda x: (-x[2], -x[1] if x[1] is not None else 0))  # Sort by rating and year
